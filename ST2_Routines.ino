// Set this to turn on Mars demo mode, e.g. display Mars time instead of local time to confuse people!
//#define DEMO
...
//    checktime();
#ifndef DEMO
    // Normal time
    writeTime(LocalHourTens, LocalHourOnes, MinTens, MinOnes);
#else
    // DEMO - Display Mars Time. Note this might display with AM/PM if in 12 hour mode,
    // which is probably a good thing.
    writeTime(MarsHourTens, MarsHourOnes, MarsMinTens, MarsMinOnes);
#endif
    break;
 ...
 //   checkDate();
    displayStringDay(LocalDays -1);
    break;

  case 3:              // Month
    displayMonth(LocalMonthCode-1);
    break;      
...
//*******************************************************************************************************************
// 								                              Display UTC Time Routine
//*******************************************************************************************************************
void DisplayUTCTimeSub()
{
#ifdef DEMO
// Don't display in demo mode
...
//*******************************************************************************************************************
// 								                              Display Mars Time Routine
//*******************************************************************************************************************
void DisplayMarsTimeSub()
{
#ifdef DEMO
// Don't display in demo mode
  NextStateRequest = true;
  ...
  if(NextSUBStateRequest)
  {
    ...
    if(SUBSTATE>8)
    {
      SUBSTATE =0; // Start at 0, not 1, here for proper setup
    }
    NextSUBStateRequest = false;
  }
  ...
  case 3:                // Start Display MSL Curiosity Time
    SUBSTATE = 4;
    blinkON = true;
    blinkFlag = false;
    blinkMin = false;
  ...
  case 6:                // Start Display MER-B Opportunity Time
    SUBSTATE = 7;
    blinkON = true;
    blinkFlag = false;
    blinkMin = false;
...
void setTimeSub()
{
  ...
  case 8:                                                          // Select 12 or 24 hour clock
    if ( blinkON ) {
      if(Display12Hour)
      {   
        displayString("12 h"); 
    ...
    if(NextSUBStateRequest)
    {
      Display12Hour = !Display12Hour;                                     
      NextSUBStateRequest = false;
      A_TH_Not24_flag = Display12Hour;
    }
    if(NextStateRequest) {
      SUBSTATE = 9;
      NextStateRequest = false;
...
void displayDate(bool UTC)
{
  ...
  if(blinkON)
  {
    ...
    for( i = 5; i <10; i++)
    {
      LEDMAT[i] = LETTERS[(UTC ? UTCDateTens : LocalDateTens) +digitoffset][y];
      y=y+1;
    }
    ...
    for( i = 10; i <15; i++)
    {
      LEDMAT[i] = LETTERS[ (UTC ? UTCDateOnes : LocalDateOnes) +digitoffset][y];
      y=y+1;
    }
...
void displayYear(bool UTC)
{
  ...
  if (blinkON) {
    for( i = 0; i <5; i++)
    {
      LEDMAT[i] = LETTERS[2+digitoffset][y];
      y=y+1;
    }
    ...
    for( i = 5; i <10; i++)
    {
      LEDMAT[i] = LETTERS[0+digitoffset][y];
      y=y+1;
    }
    ...
    for( i = 10; i <15; i++)
    {
      LEDMAT[i] = LETTERS[(UTC ? UTCYearsTens : LocalYearsTens) +digitoffset][y];
      y=y+1;
    }
    ...
    for( i = 15; i <20; i++)
    {
      LEDMAT[i] = LETTERS[ (UTC ? UTCYearsOnes : LocalYearsOnes) +digitoffset][y];
      y=y+1;
    }
  } else {
    clearmatrix();    
...
//  LEDMAT[0] = 0;
  ...
  // First, convert AM to PM if necessary
  boolean PMFlag = false;
  if ( STATE == 1 && Display12Hour ) {
     // Only do this for displaying time
     uint8_t temp = dig1 * 10 + dig2;
     if ( temp >= 12 || temp == 0 ) {
       if ( temp >= 12 ) {
         PMFlag = true;
         if ( temp > 12 )
...
//  LEDMAT[0] = 0;
  ...
  if(ALARMON && (STATE == 1 || STATE == 2 || STATE == 3))                                 // Alarm dot (top left) Do not display while setting alarm
  {
//    bitSet(LEDMAT[0],6);
  ...
  if(A_PM_NotAM_flag && (STATE == 5) && A_TH_Not24_flag)              // AM / PM dot (bottom left) (Set Alarm Time)
  {
//    bitSet(LEDMAT[0],0);
   bitSet(AMPMALARMDOTS,0);
  } 
...
void displaySol()
{
  ...
  if (blinkON) {
    ...
    for( i = 0; i <3; i++)
    {
      LEDMAT[i] = LETTERS[b+digitoffset][y];
      y=y+1;
    }
...
void displayUTCOffset()
{
  ...
  if (blinkON) {
    ...
    {
      if ( UTCOffset >= 0 )
        LEDMAT[i] = LETTERS[11][y]; // Plus sign
      else
        LEDMAT[i] = LETTERS[13][y]; // Minus sign      
      y=y+1;
    }
    ...
    if ( UTCOffset >= 0 ) {   
      Ones = UTCOffset % 10;
      Tens = (UTCOffset - Ones) / 10;
    } else {
      Ones = (UTCOffset * -1) % 10;
      Tens = ((UTCOffset * -1) - Ones) / 10;      
    }
    ...
    for( i = 10; i <15; i++)
    {
      LEDMAT[i] = LETTERS[Tens + digitoffset][y];
      y=y+1;
    }
    ...
    for( i = 15; i <20; i++)
    {
      LEDMAT[i] = LETTERS[Ones + digitoffset][y];
      y=y+1;
    }
  } else {
    clearmatrix();    
